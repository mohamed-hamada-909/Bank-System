...
public:
	...
	clsDate(short OrderDay, short Year)
	{
		clsDate Date = GetDateFromDayOrderInYear(OrderDay, Year);
	...
	//Day From Start Year
	static short DaysFromStartTheYear(short Day, short Month,short Year)
	{
		short TotalDays = 0;
	...
	short DaysFromStartTheYear()
	{
		return DaysFromStartTheYear(_Day, _Month, _Year);
	}

	//Date Add Days
	static clsDate DateAddDays(short Days, clsDate Date)
	{
		int RemainingDays = Days + DaysFromStartTheYear(Date.Day, Date.Month, Date.Year);
		short MonthDays = 0;
		Date.Month = 1;
	...
	short CalculateBusinessDays(clsDate Date2)
	{
		return CalculateBusinessDays(*this, Date2);
	}
	...
	//above method is enough, no nedd to have mothod for the object
